#' @title Horseshoe + sparse regression with Cauchy priors on variances parameters
#' @export
#'
horseshoe_core_plus <- function(Y, X, d, n, p, list_hyper, alpha_vb, c_vb, d_vb, e_vb, mu_beta_vb,
                                          sig2_beta_vb, tau_vb, tol, maxit, batch, verbose, shared_prec = F,
                                          full_output = FALSE) {
  ...
  with(list_hyper, { # list_init not used with the with() function to avoid
    ...
    while ((!converged) & (it <= maxit)) {
      ...
      # % # update of sigma^{-2}
      lambda_vb <- (p*d+1)/2
      nu_vb <- update_nu_vb_horseshoe_plus(b_inv_vb, m2_beta, alpha_vb, tau_vb, shared_prec)
      ...
      kappa_vb <- update_kappa_vb_horseshoe_plus(Y, X,  mat_x_m1, alpha_vb,
      ...
      lb_new <- lower_bound_horseshoe_plus(Y, X, d, n, p, sig2_beta_vb, sig2_inv_vb, tau_vb,
                                                c_vb, alpha_vb, mat_x_m1, mu_beta_vb,
                                                m2_beta, G_vb, nu_vb, kappa_vb, a_inv_vb, b_inv_vb, d_vb, e_vb, A, B, shared_prec)
      ELBO <- c(ELBO, lb_new)
    ...
    if (full_output) { # for internal use only
      create_named_list_(mu_beta_vb, sig2_beta_vb, sig2_inv_vb, tau_vb,c_vb,
                         alpha_vb, c_vb, eta_vb, kappa_vb, lambda_vb, nu_vb, b_inv_vb,
      ...
      create_named_list_(lb_opt, mu_beta_vb, alpha_vb,
...
update_nu_vb_horseshoe_plus <- function(b_inv_vb, m2_beta, alpha_vb, tau_vb, shared_prec) {
...
update_kappa_vb_horseshoe_plus <- function(Y_mat, X_mat, mat_x_m1, alpha_vb, d, n, p, m1_beta,
...
lower_bound_horseshoe_plus <- function(Y, X, d, n, p, sig2_beta_vb, sig2_inv_vb, tau_vb,
  ...
  log_c_vb <- digamma(1) - log(alpha_vb + d_vb)
  log_d_vb <- digamma(1) - log(e_vb + c_vb)
  log_e_vb <- digamma(1) - log(d_vb + 1)
  ...
  L_H_7 <- sum(alpha_vb*G_vb - lgamma(1/2) +
                (1/2)*log_c_vb - log(c_vb + G_vb))

  # this is for c
  L_H_8 <- sum(-(1/2)*log_c_vb + c_vb*alpha_vb - lgamma(1/2) +
               (1/2)*log_d_vb - log(d_vb + alpha_vb))

  # this is for d
  L_H_9 <- sum(-(1/2)*log_d_vb + d_vb*c_vb - lgamma(1/2)
                 +(1/2)*log_e_vb - log(e_vb + c_vb))

  # this is for e
  L_H_10 <- sum(-(1/2)*log_e_vb + e_vb*d_vb - lgamma(1/2) -
                 log(1 + d_vb))

  l <- L_1 + (L_2 - H_2) + L_H_3 + L_H_4 + L_H_5 + L_H_6 +
       L_H_7 + L_H_8 +  L_H_9 + L_H_10
